...
{
	if (const SharedPtr<PlayerSpaceshipController> controller = get_player_from_gamepad_id(gamepad_id))
	{
	...
	SharedPtr<PlayerSpaceshipController> controller = engine.create_entity<PlayerSpaceshipController>(input_context);
...
SharedPtr<PlayerSpaceshipController> PlayerManager::get_player_from_gamepad_id(int gamepad_id) const
{
...
{
	create_player(Vec3 { 0.0f, -100.0f * gamepad_id, 0.0f }, Quaternion::identity, gamepad_id);
}

void PlayerManager::on_gamepad_disconnected(const int gamepad_id)
{
	const SharedPtr<PlayerSpaceshipController> controller = get_player_from_gamepad_id(gamepad_id);
